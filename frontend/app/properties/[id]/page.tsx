...
import {
  ...
  Calendar, FileText, Sparkles, Copy, Mail, Phone, User, Building2, Bookmark
} from "lucide-react";
...
export default function PropertyDetailPage() {
  ...
  const handleEmail = () => {
    if (!analysis || !property) return;
    // Copy full analysis to clipboard then open Gmail compose in a new tab
    navigator.clipboard.writeText(
      `Investment Analysis: ${property.address}, ${property.city}, ${property.state}\n` +
      `Asking Price: ${property.asking_price ? "$" + property.asking_price.toLocaleString() : "N/A"}\n\n` +
      `${analysis}\n\n— Generated by Waco Deal Finder`
    );
    const subject = encodeURIComponent(`Investment Analysis: ${property.address}`);
    const body = encodeURIComponent(
      `Hi,\n\nPlease see the investment analysis for ${property.address} below.\n\n` +
      `[Analysis copied to clipboard — press Ctrl+V to paste]\n\n` +
      `— Generated by Waco Deal Finder`
    );
  ...
  useEffect(() => {
    if (!id) return;
    api.getProperty(id)
      .then((p) => { setProperty(p); setSaved(p.is_saved); })
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3 mt-4 flex-wrap">
            ...
            >
              <Bookmark size={15} className={saved ? "fill-amber-400" : ""} />
              {saved ? "Saved" : "Save"}
            </button>
            {p.listing_url && (
              <a
